...
export const BASE_DIR = path.resolve(process.env.FOLDER_BROWSER_PATH || '/srv/data');
...
export async function list(relPath = '.'): Promise<{ name: string; isDirectory: boolean }[]> {
    await ensureBaseDir();
...
export async function create(relPath: string, content = ''): Promise<{ created: string }> {
...
export async function remove(relPath: string): Promise<{ removed: string }> {
    const filePath = safePath(relPath);
    await fs.rm(filePath, { recursive: true, force: true });
    return { removed: relPath };
}

export async function copy(srcRelPath: string, destRelPath: string): Promise<{ copied: string }> {
...
export async function move(srcRelPath: string, destRelPath: string): Promise<{ moved: string }> {
    const srcPath = safePath(srcRelPath);
    const destPath = safePath(destRelPath);
    const stat = await fs.stat(srcPath).catch(() => null);
    if (!stat) throw new Error('Source not found');
